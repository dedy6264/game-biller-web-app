...
// ConvertIAKInquiryResponse converts IAK inquiry response code (rc) to main response format.
func ConvertIAKInquiryResponse(rc string) (mainCode string, providerMsg string, mainMsg string) {
	if resp, exists := respInq[rc]; exists {
		return resp.Maincode, resp.ProviderMsg, resp.MainMsg
	}
	return CodeErrPvd4305, "UNKNOWN ERROR", "Upstream provider responded with an unmapped error structure."
}
