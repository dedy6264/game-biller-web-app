...
func AdminDeleteMerchant(c echo.Context) error {
	var (
		svc = "AdminDeleteMerchant"
		req struct {
			ID int64 `json:"id"`
		}
	)
	...
	err := repositories.DeleteMerchant(connections.DBconn(), req.ID)
	if err != nil {
		helpers.ProcessLogger(c, svc, err.Error(), "Failed to delete merchant")
